...
export default function TarikScreen() {
  ...
  const [selectedRek, setSelectedRek] = useState<any>(null); // rekening tujuan (digital, bertambah)
  const [selectedCashRek, setSelectedCashRek] = useState<any>(null); // sumber cash (berkurang)
  const [channelCustomer, setChannelCustomer] = useState(''); // hanya catatan: customer kirim dari mana
  ...
  const cashKurang = !!(selectedCashRek && nominal && Number(nominal) > 0 && selectedCashRek.saldo < Number(nominal));
  ...
  const simpan = async () => {
    ...
    if (!selectedRek) return Alert.alert('Error', 'Pilih rekening yang menerima transfer customer');
    if (!selectedCashRek) return Alert.alert('Error', 'Pilih sumber cash yang diberikan ke customer');
    ...
    if (selectedCashRek.saldo < Number(nominal)) {
      return Alert.alert(
        '⚠️ Modal Tidak Cukup',
        `Modal di rekening "${selectedCashRek.nama}" tidak cukup untuk transaksi ini.\n\nSaldo tersedia: ${fmtRp(selectedCashRek.saldo)}\nDibutuhkan: ${fmtRp(Number(nominal))}\nKurang: ${fmtRp(Number(nominal) - selectedCashRek.saldo)}`
      );
    }
    ...
    try {
      ...
      const { data: freshCash, error: errFresh } = await supabase
        .from('rekening').select('saldo, nama').eq('id', selectedCashRek.id).single();
      if (errFresh || !freshCash) throw new Error('Gagal memverifikasi saldo terbaru, coba lagi.');
      if (freshCash.saldo < nominalNum) {
        setLoading(false);
        return Alert.alert(
          '⚠️ Modal Tidak Cukup',
          `Modal di rekening "${freshCash.nama}" tidak cukup untuk transaksi ini.\n\nSaldo tersedia: ${fmtRp(freshCash.saldo)}\nDibutuhkan: ${fmtRp(nominalNum)}\nKurang: ${fmtRp(nominalNum - freshCash.saldo)}`
        );
      }

      const newSaldoRek = selectedRek.saldo + nominalNum;
      const newSaldoCash = freshCash.saldo - nominalNum;

      const { error: errRek } = await supabase.from('rekening')
        .update({ saldo: newSaldoRek, updated_at: new Date().toISOString() })
      ...
      const { error: errCash } = await supabase.from('rekening')
        .update({ saldo: newSaldoCash, updated_at: new Date().toISOString() })
      ...
      const { error: errTx } = await supabase.from('transaksi').insert({
        user_id: user.id, jenis: 'TU', nominal: nominalNum,
      ...
      Alert.alert('✅ Berhasil', `Tarik Tunai ${fmtRp(nominalNum)} berhasil disimpan!`);
      resetForm();
      loadData();
    } catch (e: any) {
  ...
  return (
    <View style={styles.root}>
      <LinearGradient colors={GRADIENT_HEADER} style={styles.header}>
        <Text style={styles.headerTitle}>💸 Tarik Tunai (TU)</Text>
        <Text style={styles.headerSub}>Customer transfer digital → kamu kasih uang cash</Text>
      </LinearGradient>

      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">

        <View style={styles.card}>
          <Text style={styles.cardTitle}>NOMINAL PENARIKAN</Text>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>BIAYA ADMIN</Text>
        ...
        <View style={styles.card}>
          <View style={styles.stepRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.stepHelper}>Rekening yang menerima transfer dari customer (mis. DANA kamu).</Text>
            </View>
          </View>
          <RekeningDropdown
            label=""
          ...
          <ChannelPicker
            label="Customer kirim dari mana?"
            helper='Cuma catatan, TIDAK ngaruh ke saldo. Contoh: customer transfer dari BCA ke rekening di atas. Kalau sesama platform (mis. DANA ke DANA), boleh dikosongkan.'
        ...
        <View style={styles.card}>
          <View style={styles.stepRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.stepHelper}>Sumber cash fisik yang kamu serahkan ke customer.</Text>
            </View>
          </View>
          <RekeningDropdown
            label=""
            placeholder="Pilih sumber cash"
            value={selectedCashRek}
            options={rekening.filter(r => r.id !== selectedRek?.id)}
            onSelect={setSelectedCashRek}
            accent={colors.red}
          />
          {selectedCashRek && nominal && Number(nominal) > 0 && selectedCashRek.saldo < Number(nominal) ? (
            <View style={styles.warnBox}>
              <Text style={styles.warnTitle}>⚠️ Modal tidak cukup</Text>
              <Text style={styles.warnText}>
                Saldo "{selectedCashRek.nama}" hanya {fmtRp(selectedCashRek.saldo)}, dibutuhkan {fmtRp(Number(nominal))}.
              </Text>
            </View>
          ) : null}
        </View>
        ...
        <View style={styles.card}>
          ...
          <TextInput
            style={styles.input} placeholder="Contoh: Nasabah DANA - Ibu Sari"
            placeholderTextColor={colors.gray400}
        ...
        {nominal ? (
          <View style={styles.preview}>
            ...
            <View style={styles.previewLine}>
              <Text style={styles.previewLabel}>⬆️ {selectedRek?.nama || 'Rekening penerima'} bertambah</Text>
              <Text style={[styles.previewValue, { color: colors.green }]}>{fmtRp(Number(nominal))}</Text>
            </View>
            <View style={styles.previewLine}>
              <Text style={styles.previewLabel}>⬇️ {selectedCashRek?.nama || 'Sumber cash'} berkurang</Text>
              <Text style={[styles.previewValue, { color: colors.red }]}>{fmtRp(Number(nominal))}</Text>
            </View>
            {channelCustomer ? (
              <View style={styles.previewLine}>
                <Text style={styles.previewLabel}>ℹ️ Customer kirim dari</Text>
        ...
        <TouchableOpacity
          style={[styles.submitBtn, (loading || cashKurang || !isValid) && { opacity: 0.5 }]}
          onPress={simpan}
          disabled={loading || cashKurang || !isValid}
        >
          {loading ? <ActivityIndicator color={colors.white} /> : (
            <>
              <Ionicons name="checkmark-circle" size={18} color={colors.white} />
              <Text style={styles.submitBtnText}>Simpan Tarik Tunai</Text>
            </>
          )}
        </TouchableOpacity>
      </ScrollView>
...
function createStyles(colors: ReturnType<typeof useTheme>['colors']) {
  ...
  submitBtn: { flexDirection: 'row', gap: 8, backgroundColor: colors.red, padding: 16, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
